...
public class ResourcesLoaderTest {
	...
	public void testLoadResources_ZipFile_lazy_all() throws IOException {
		...
		// then
		verifyResources(resources);
		Assertions.assertEquals(Resource.class, resources.getById("container").getClass());
		Assertions.assertEquals(LazyResource.class, resources.getById("book1").getClass());
	}
	...
	public void testLoadResources_ZipFile_partial_lazy() throws IOException {
		...
		Assertions.assertEquals(Resource.class, resources.getById("container").getClass());
		Assertions.assertEquals(LazyResource.class, resources.getById("book1").getClass());
		Assertions.assertEquals(Resource.class, resources.getById("chapter1").getClass());
	}

	private void verifyResources(Resources resources) throws IOException {
		Assertions.assertNotNull(resources);
		Assertions.assertEquals(12, resources.getAll().size());
		...
		Assertions.assertEquals("container", resource.getId());
		Assertions.assertEquals("META-INF/container.xml", resource.getHref());
		Assertions.assertNull(resource.getMediaType());
		Assertions.assertEquals(230, resource.getData().length);
		
		// book1.css
		resource = resources.getByHref(allHrefs.get(1));
		Assertions.assertEquals("book1", resource.getId());
		Assertions.assertEquals("OEBPS/book1.css", resource.getHref());
		Assertions.assertEquals(MediatypeService.CSS, resource.getMediaType());
		Assertions.assertEquals(65, resource.getData().length);
		expectedData = IOUtil.toByteArray(this.getClass().getResourceAsStream("/book1/book1.css"));
		Assertions.assertArrayEquals(expectedData, resource.getData());
		
		
		// chapter1
		resource = resources.getByHref(allHrefs.get(2));
		Assertions.assertEquals("chapter1", resource.getId());
		Assertions.assertEquals("OEBPS/chapter1.html", resource.getHref());
		Assertions.assertEquals(MediatypeService.XHTML, resource.getMediaType());
		Assertions.assertEquals(247, resource.getData().length);
